    ...
    <nav className="bg-white border-b border-black/[0.07] sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-syne font-black text-xl text-ink">Pack<span className="text-accent">Vista</span></span>
        </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/contact" className="hidden md:block bg-accent text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-orange-700 transition-colors">
            Get Quote
          </Link>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-black/[0.06] px-4 py-3 flex flex-col gap-2">
          {links.map(l => (
            <Link key={l.to} to={l.to} onClick={() => setOpen(false)} className="py-2 text-sm font-medium text-ink2 hover:text-accent">{l.label}</Link>
          ))}
        </div>
      )}
    </nav>
